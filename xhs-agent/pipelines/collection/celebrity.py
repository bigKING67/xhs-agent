...
class CelebAggregator(BaseCollector[CelebData]):
    ...
    async def _get_user_notes(
        ...
        """
        try:
            result = self._xhs_client.get_user_posts(user_id, limit=limit)
    ...
    def _calculate_celeb_stats(
        ...
        followers = user_info_detail.get("followers", 0)

        # 计算互动统计
        if notes:
            avg_likes = sum(
                n.get("interact_info", {}).get("likes", 0)
                for n in notes
            ) / len(notes)
            avg_comments = sum(
                n.get("interact_info", {}).get("comments", 0)
                for n in notes
            ) / len(notes)
            avg_shares = sum(
                n.get("interact_info", {}).get("shares", 0)
                for n in notes
            ) / len(notes)
        else:
        ...
        return CelebData(
            celeb_id=user_info_detail.get("user_id", ""),
            name=user_info_detail.get("nickname", ""),
            avatar_url=user_info_detail.get("avatar", ""),
            followers=followers,
        ...
        for note in notes:
            ...
            if isinstance(published_at, str):
                try:
                    ...
                    published_times.append(publish_time)
